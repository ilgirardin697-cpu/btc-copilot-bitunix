...
"""
BTCUSDT COPILOT + TELEGRAM — Bitunix public data only.
...
class BitunixPublic:
    def __init__(self):
        ...
        self.s.headers.update({"User-Agent": "BTC-Copilot-Telegram/1.0"})
        self.last_req = 0.0
...
class Analyzer:
    ...
    def live_snapshot(self):
        ...
        # REST fallback for current price/funding/depth
        ...
        price = ws["price"] if ws_fresh and finite(ws["price"]) else num(tick.get("lastPrice") or tick.get("last"))
        funding = ws["funding"] if ws_fresh and finite(ws["funding"]) else num(fund.get("fundingRate"))
    ...
    def make_targets(self, lev, entry, side, atr1h):
        if side == "LONG":
            candidates = sorted(x for x in lev if x > entry*1.001)
            fallback = [entry + atr1h*m for m in (1.0, 2.0, 3.2)]
        else:
            candidates = sorted((x for x in lev if x < entry*0.999), reverse=True)
            fallback = [entry - atr1h*m for m in (1.0, 2.0, 3.2)]
        out = []
    ...
    def analyze(self) -> Plan:
        ...
        w, d, h4 = t["1w"], t["1d"], t["4h"]

        if d == "BULL" and h4 in ("BULL", "BULL_SOFT") and w != "BEAR":
            bias = "LONG STRONG" if w in ("BULL", "BULL_SOFT") and h4 == "BULL" else "LONG"
        elif d == "BEAR" and h4 in ("BEAR", "BEAR_SOFT") and w != "BULL":
            bias = "SHORT STRONG" if w in ("BEAR", "BEAR_SOFT") and h4 == "BEAR" else "SHORT"
        elif w in ("BULL", "BULL_SOFT") and d in ("BULL", "BULL_SOFT") and h4 != "BEAR":
            bias = "LONG"
        elif w in ("BEAR", "BEAR_SOFT") and d in ("BEAR", "BEAR_SOFT") and h4 != "BULL":
            bias = "SHORT"
        else:
            bias = "NEUTRAL"

        x4, x1, x15, p15, x5 = f["4h"].iloc[-1], f["1h"].iloc[-1], f["15m"].iloc[-1], f["15m"].iloc[-2], f["5m"].iloc[-1]
        lev = levels(f, price)
        ...
        # Context is deliberately separate from entry permission.
        if bias.startswith("LONG") and t["1h"].startswith("BEAR"):
            context = "1H correction/pullback inside bullish higher-timeframe context"
        elif bias.startswith("SHORT") and t["1h"].startswith("BULL"):
            context = "1H rebound inside bearish higher-timeframe context"
        else:
            context = "Intraday momentum broadly aligned with higher-timeframe context"

        if bias == "NEUTRAL":
            wait.append("No dominant 1D/4H alignment. Wait for structure.")
            return Plan(datetime.now(TZ), price, bias, action, setup, context, None, None, None, None, None, None, None, None, None, None, why, wait, warnings, t, float(x1.rsi), float(x4.adx), float(x1.adx), snap["funding"], snap["book"], snap["flow"])

        side = "LONG" if bias.startswith("LONG") else "SHORT"

        # Macro vetoes: a local RSI cannot reverse the higher-timeframe bias.
        if side == "LONG":
            why += [
                f"1D {t['1d']} / 4H {t['4h']} / structure 4H {st['4h']}",
                f"ADX 4H {x4.adx:.1f}, +DI {x4.pdi:.1f} vs -DI {x4.mdi:.1f}",
            ]
            breakout = float(f["15m"].high.iloc[-21:-1].max())
            pull_center = max([x for x in (sup, float(x15.ema20), float(x15.vwap)) if finite(x) and x < price*1.004], default=price-0.7*atr15)
            pull_lo, pull_hi = pull_center-0.20*atr15, pull_center+0.20*atr15
            ...
            pullback_ok = (
                x15.low <= max(x15.ema20, x15.vwap) + 0.15*atr15
            ...
            if div_veto:
                warnings.append("Bearish divergence on both 1H and 15m: no LONG confirmation")

            candidate = breakout_ok or pullback_ok
            if candidate and not div_veto:
                setup = "BREAKOUT" if breakout_ok else "PULLBACK / RECLAIM"
                entry_low, entry_high = price-0.10*atr15, price+0.10*atr15
                swing_stop = sup if sup else float(f["15m"].low.iloc[-12:].min())
                stop = min(float(x15.ema50), swing_stop) - 0.15*atr15
                tp1, tp2, tp3 = self.make_targets(lev, price, "LONG", atr1)

                risk = max(price-stop, 1e-9)
                rr2 = (tp2-price)/risk if tp2 else None
                extension = (price-float(x15.ema20))/max(atr15, 1e-9)

                if extension > MAX_EXTENSION_ATR or (rr2 is not None and rr2 < MIN_RR_LATE):
                    action = "TOO LATE"
                    warnings.append("Bullish context, but price is too extended / remaining R:R is poor")
                elif rr2 is not None and rr2 >= MIN_RR_ENTER:
                    action = "ENTER LONG NOW"
                else:
                    action = "WAIT"
                    wait.append("Need more room to TP2 or a better pullback entry")
            else:
                action = "WAIT"
                wait.append(f"Breakout: 15m close above {price_fmt(breakout)} with volume/momentum")
                wait.append(f"Or pullback/reclaim around {price_fmt(pull_lo)}–{price_fmt(pull_hi)}")
                if t["1h"].startswith("BEAR"):
                    wait.append("1H is correcting: do not turn that alone into a SHORT")

        else:
            why += [
                f"1D {t['1d']} / 4H {t['4h']} / structure 4H {st['4h']}",
                f"ADX 4H {x4.adx:.1f}, -DI {x4.mdi:.1f} vs +DI {x4.pdi:.1f}",
            ]
            breakout = float(f["15m"].low.iloc[-21:-1].min())
            pull_center = min([x for x in (res, float(x15.ema20), float(x15.vwap)) if finite(x) and x > price*0.996], default=price+0.7*atr15)
            pull_lo, pull_hi = pull_center-0.20*atr15, pull_center+0.20*atr15

            # Shorts intentionally stricter on BTC.
            macro_short_ok = (
                t["1d"] == "BEAR"
            ...
            pullback_ok = (
                macro_short_ok
                and x15.high >= min(x15.ema20, x15.vwap) - 0.15*atr15
            ...
            candidate = breakout_ok or pullback_ok
            if candidate and not div_veto:
                setup = "BREAKDOWN" if breakout_ok else "PULLBACK / REJECTION"
                entry_low, entry_high = price-0.10*atr15, price+0.10*atr15
                swing_stop = res if res else float(f["15m"].high.iloc[-12:].max())
                stop = max(float(x15.ema50), swing_stop) + 0.15*atr15
                tp1, tp2, tp3 = self.make_targets(lev, price, "SHORT", atr1)

                risk = max(stop-price, 1e-9)
                rr2 = (price-tp2)/risk if tp2 else None
                extension = (float(x15.ema20)-price)/max(atr15, 1e-9)

                if extension > MAX_EXTENSION_ATR or (rr2 is not None and rr2 < MIN_RR_LATE):
                    action = "TOO LATE"
                    warnings.append("Bearish context, but price is too extended / remaining R:R is poor")
                elif rr2 is not None and rr2 >= MIN_RR_ENTER:
                    action = "ENTER SHORT NOW"
                else:
                    action = "WAIT"
                    wait.append("Need more room to TP2 or a better rejection entry")
            else:
                action = "WAIT"
                wait.append(f"Breakdown: 15m close below {price_fmt(breakout)} with volume/momentum")
                wait.append(f"Or pullback/rejection around {price_fmt(pull_lo)}–{price_fmt(pull_hi)}")
                if t["1h"].startswith("BULL"):
                    wait.append("1H is rebounding: do not chase a SHORT until rejection confirms")

        # Microstructure is supporting evidence, never the macro direction.
        if snap["book"] is not None:
            if side == "LONG" and snap["book"] >= 0.56:
                why.append(f"Order-book bid share ~{snap['book']*100:.0f}% supports LONG")
            elif side == "SHORT" and snap["book"] <= 0.44:
                why.append(f"Order-book bid share ~{snap['book']*100:.0f}% supports SHORT")
        if snap["flow"] is not None:
            if side == "LONG" and snap["flow"] >= 0.57:
                why.append(f"Recent buy flow ~{snap['flow']*100:.0f}%")
            elif side == "SHORT" and snap["flow"] <= 0.43:
                why.append(f"Recent sell flow ~{(1-snap['flow'])*100:.0f}%")

        return Plan(
            datetime.now(TZ), price, bias, action, setup, context,
...
def plan_message(p: Plan, title="BTCUSDT COPILOT") -> str:
    lines = [
        f"<b>{html.escape(title)}</b>",
        f"Precio: <b>{price_fmt(p.price)} USDT</b>",
        "",
        f"{icon_bias(p.bias)} Sesgo: <b>{html.escape(p.bias)}</b>",
        f"{icon_action(p.action)} Estado: <b>{html.escape(p.action)}</b>",
    ...
    if p.entry_low is not None:
        lines += [
            "",
            f"Entrada: <b>{price_fmt(p.entry_low)}–{price_fmt(p.entry_high)}</b>",
            ...
            f"R:R restante a TP2: <b>{p.rr2:.2f}R</b>" if p.rr2 is not None else "",
        ]

    if p.wait:
        lines += ["", "<b>Qué espera:</b>"]
        lines += [f"• {html.escape(x)}" for x in p.wait[:4]]

    if p.why:
        lines += ["", "<b>Por qué:</b>"]
        lines += [f"• {html.escape(x)}" for x in p.why[:5]]

    if p.warnings:
        lines += ["", "<b>Avisos:</b>"]
        lines += [f"• {html.escape(x)}" for x in p.warnings[:4]]

    lines += [
        "",
        "<i>Sesgo ≠ entrada. Solo ENTER NOW significa setup confirmado por las reglas.",
        "No es una garantía ni una orden automática.</i>",
    ]
    return "\n".join(x for x in lines if x != "" or True)[:4090]


def startup_message(p: Plan) -> str:
    return (
        "🤖 <b>BTC Copilot conectado</b>\n\n"
        "Fuente: Bitunix BTCUSDT Futures\n"
        "Modo: SOLO análisis; no puede operar tu cuenta.\n"
        "Alertas: ENTER NOW + cambio de sesgo + plan diario.\n\n"
        + plan_message(p, "ESTADO INICIAL")
    )
...
def main():
    ...
    log("BTC COPILOT TELEGRAM starting...")
